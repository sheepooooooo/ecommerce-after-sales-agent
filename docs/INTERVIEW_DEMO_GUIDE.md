...
4. 缺订单号后的任务续办，60 秒
   演示问题：先输入 `帮我退款`，再用同一 `session_id` 输入 `ORD10003`。
   关注点：第一轮只追问并保存 `pending_action=refund_eligibility`；第二轮恢复退款资格判断，而不是把订单号当成新的普通查询。

5. 复合任务进入工单确认，90 秒
   演示问题：`ORD10003 能退款吗？如果不能，帮我创建售后工单。`  
   关注点：系统先判断退款资格；不可直接退款时只返回 `ticket_confirmation_required`，不立即写库。

6. 重复确认幂等与 trace 查询，60 秒
...
- 当前离线 Agent 评测为 79 条，端到端通过 77 条，保留 ASAE007、ASAE016 badcase。
